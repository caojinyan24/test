 ...
 * @author hanhan.zhang
 * */
public class QMultiThreadServer {
    ...
    // 工作线程
//    private Set<Thread> workerThreads = new HashSet<Thread>();
...
//        workerThreads.add(socketThread);
    ...
    private void doRead(SelectionKey key) throws IOException {
        ...
        // 读取数据包头
        ByteBuffer buffer = ByteBuffer.allocate(4);//包头是4个字节
        sc.read(buffer);
        if (buffer.remaining() == 0) {
            ...
            if (buffer.remaining() == 0) {
                ...
                System.out.println("socket thread receive : " + new String(buffer.array()));
                // 响应客户端(注册写事件)
                preWrite(key);
            }
        }
    ...
    public static void main(String[] args) throws IOException {
        QMultiThreadServer multiThreadServer = new QMultiThreadServer();
